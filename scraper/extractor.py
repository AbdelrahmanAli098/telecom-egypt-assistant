...
# Template detection — decides which extractor a page needs
def detect_template(soup):
    if soup.select(".expandable_question"):
        return "faq"
    elif soup.select(".contractual_terms"):
        return "legal"
    elif soup.select(".journal-content-article table"):
        return "table"
    else:
        return "plain"


# 1. Generic / plain content extractor (fixed version — no dropped text)
...
# 3. Legal / contractual terms extractor (recursive nested clauses)
    ...
    with sync_playwright() as p:
        ...
        for i, item in enumerate(pages, start=1):
            ...
            try:
                ...
                if template == "faq":
                    page_records = extract_faq_accordion(
                        soup, url, item["language"], item["category"], title
                    )
                elif template == "legal":
                    page_records = extract_legal_content(
                        soup, url, item["language"], item["category"], title
                    )
                else:
                    ...
                    if len(content) < 30:
                        ...
                        page_records = [{
                            ...
                            "title": title,
                            "content": content,
                            "content_type": template,  # "plain" or "table"
                            "scraped_at": str(date.today()),
                        }]

                results.extend(page_records)
                print(f"   OK ({template}, {len(page_records)} record(s))")
